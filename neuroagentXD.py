...
def draw_pause_menu(window, font, SCREEN_WIDTH, SCREEN_HEIGHT):
    """Рисует меню паузы с подсказками по клавишам"""
    menu_width = 500
    menu_height = 430
    ...
    pygame.draw.rect(menu_bg, WHITE, (0, 0, menu_width, menu_height), 2)
    window.blit(menu_bg, (menu_x, menu_y))
    
    title = font.render("МЕНЮ ПАУЗЫ", True, WHITE)
    window.blit(title, (menu_x + menu_width // 2 - 80, menu_y + 20))
    
    controls = [
        "SPACE: Продолжить/остановить агента",
        "R: Новый лабиринт",
...
def update_stats_plot(rewards_history, steps_history, episodes_list):
    ...
    plt.subplot(1, 2, 1)
    plt.plot(episodes_list, rewards_history, label='Награда')
    plt.xlabel('Эпизоды')
    plt.ylabel('Награда')
    ...
    plt.subplot(1, 2, 2)
    plt.plot(episodes_list, steps_history, label='Шаги', color='orange')
    plt.xlabel('Эпизоды')
    plt.ylabel('Шаги')
...
if __name__ == "__main__":
    ...
    training_episodes = 0
    paused = False
    show_menu = False  # Флаг для показа меню паузы
    agent_speed = 100  # Начальная скорость агента в мс
    agent_color = BLUE  # Цвет агента
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_SPACE:
                    paused = not paused
                    message = "Продолжено" if not paused else "Пауза"
                elif event.key == pygame.K_ESCAPE:
                    show_menu = not show_menu
                    if show_menu and not paused:
                        paused = True  # Автоматически ставим на паузу при открытии меню
                        message = "Меню паузы"
                    ...
                    saved_agent_pos = agent_pos.copy()
                    saved_target_pos = target_pos.copy()
                    message = "Лабиринт сохранен (L)"
                elif event.key == pygame.K_k and paused:
                    # Загрузить лабиринт
                    if saved_grid is not None:
                        env.grid = saved_grid.copy()
                        grid = saved_grid.copy()
                        agent_pos = saved_agent_pos.copy()
                        target_pos = saved_target_pos.copy()
                        env.agent_pos = agent_pos.copy()
                        env.target_pos = target_pos.copy()
                        env.visited = set([tuple(agent_pos)])
                        env.position_history = deque(maxlen=50)
                        env.position_history.append(tuple(agent_pos))
                        env.current_plan = []
                        env.plan_index = 0
                        env.think_count = 0
                        env.failed_plan_count = 0
                        obs = grid.copy()
                    ...
                    # Цикл цветов агента
                    colors = [BLUE, RED, GREEN, (255, 255, 0), (255, 0, 255), (0, 255, 255)]
                    current_index = colors.index(agent_color) if agent_color in colors else 0
                    agent_color = colors[(current_index + 1) % len(colors)]
                    message = f"Цвет агента изменен"
        ...
        for row in range(current_grid_size):
            for col in range(current_grid_size):
                ...
                if cell_val == 1:   
                    ...
                    pygame.draw.circle(window, agent_color, rect.center, CELL_SIZE // 2 - 2)
                elif cell_val == 4: 
        ...
        if show_menu:
            draw_pause_menu(window, font, SCREEN_WIDTH, SCREEN_HEIGHT)
